...
// MEMORY TRANSFER OPTIMIZATION: Use pinned memory for faster transfers
DTYPE* h_arr_pinned;
...
// Copy data from host to device with pinned memory (much faster)
...
// MEMORY TRANSFER OPTIMIZATION: Use pinned memory for D2H transfer
CUDA_CHECK(cudaMemcpy(h_arr_pinned, d_arr, size * sizeof(DTYPE), cudaMemcpyDeviceToHost));
