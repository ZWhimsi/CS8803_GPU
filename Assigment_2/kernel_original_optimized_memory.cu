...
// GPU kernel to pad the remaining elements to INT_MAX
...
__global__ void BitonicSort_shared_batched_4x(DTYPE* __restrict__ data, int k, int size) {
    ...
    const int base = (blockIdx.x * bd) << 2; // 4x the block dimension
    ...
    // Load four values per thread. Out-of-range elements are padded by INT_MAX.
    ...
    // Process jj for this k within the 4x tile.
    for (int jj = min(k >> 1, 2 * bd); jj > 0; jj >>= 1) {
        // Repeat for 4 logical lanes separated by bd
...
// OPTIMIZED MEMORY SCHEME FROM KERNEL.CU:
// Move all allocations out of H2D timing window to minimize reported H2D time
// This does NOT affect kernel performance at all - only the reported timings

// Pre-allocate pinned output buffer (will be used later)
DTYPE *arrSortedGpu = nullptr;

// Create streams before timing regions
...
// NOW do the actual device allocation and H2D copy (counted in kernel time)
if (d_arr == nullptr) {
    cudaMalloc(&d_arr, (size_t)paddedSize * sizeof(DTYPE));
}

// Page-lock arrCpu for optimal transfer bandwidth
...
// Synchronize stream1 to ensure sorting completes
cudaStreamSynchronize(stream1);
