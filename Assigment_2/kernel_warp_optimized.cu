...
// Optimized kernel for small j values using warp shuffles
__global__ void BitonicSort_warp(int* data, int j, int k, int size) {
    ...
    if (j < 32) {
        ...
        // Compare and select
        bool swap = (val > partner_val) == ascending;
        
        // Only lower index threads write (to avoid conflicts)
        if ((threadIdx.x & j) == 0) {
            data[i] = swap ? partner_val : val;
            data[i ^ j] = swap ? val : partner_val;
        }
    } else {
        // Fall back to regular implementation for larger j
        int partnerIdx = i ^ j;
...
// GPU Kernel for bitonic sort with global memory (optimized)
...
// GPU Kernel for bitonic sort with shared memory (optimized)
...
int main(int argc, char *argv[]) {
    ...
    int threadsPerBlock = 1024;  // Max threads for better occupancy
    ...
    for (int k = 2; k <= size; k *= 2) {
        for (int j = k/2; j > 0; j /= 2) {
            // Choose kernel based on j value and array size
            if (j < 32) {
                // Use warp shuffle optimization for small j
                BitonicSort_warp<<<blocksPerGrid, threadsPerBlock>>>(d_arr, j, k, size);
                warp_steps++;
            } else if (j < threadsPerBlock && size <= 1024) {
                // Use shared memory for medium j when beneficial
                BitonicSort_shared<<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(d_arr, j, k, size);
                shared_steps++;
            } else {
                // Use global memory for large j
    ...
    LOG_INFO("Kernel breakdown: %d warp steps, %d shared steps, %d global steps", 
             warp_steps, shared_steps, global_steps);
