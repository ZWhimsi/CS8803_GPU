...
// OPTIMIZATION: Fused kernel that processes multiple j values in one launch
__global__ void BitonicSort_fused(int* data, int start_j, int end_j, int k, int size) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    
    // Process multiple j values in a single kernel
    for (int j = start_j; j >= end_j; j /= 2) {
        int i = tid;
        if (i >= size) continue;
        ...
        // Synchronize within grid for next j iteration
        // Note: This requires cooperative groups on newer GPUs
        __syncthreads();
        if (blockIdx.x == 0 && threadIdx.x == 0 && j > end_j) {
            // Simple grid-wide sync (not perfect but reduces launches)
            __threadfence();
        }
    }
}

// Optimized global kernel with unrolled comparisons
__global__ void BitonicSort_global_unrolled(int* data, int j, int k, int size){
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    
    // Process multiple elements per thread to improve efficiency
    #pragma unroll 4
...
int main(int argc, char *argv[]) {
    ...
    // Allocate memory for GPU result using pinned memory
    ...
    // Perform bitonic sort on GPU with kernel fusion optimization
    LOG_STAGE("Starting kernel fusion bitonic sort on GPU");
    
    // OPTIMIZATION: Use fewer blocks to reduce overhead
    int threadsPerBlock = 1024;  // Maximum threads
    int blocksPerGrid = min(65535, (size + threadsPerBlock - 1) / threadsPerBlock);
    ...
    int fused_launches = 0;
    
    // Kernel fusion strategy: combine multiple j iterations when possible
    for (int k = 2; k <= size; k *= 2) {
        int j = k/2;
        
        // For large j values, try to fuse multiple iterations
        if (j >= 512) {
            // Calculate how many j values we can safely fuse
            int end_j = max(512, j/8);  // Fuse up to 8 iterations
            
            BitonicSort_fused<<<blocksPerGrid, threadsPerBlock>>>(d_arr, j, end_j, k, size);
            fused_launches++;
            total_launches++;
            
            // Continue with remaining j values
            j = end_j / 2;
        }
        
        // Process remaining j values with optimized kernel
        while (j > 0) {
            if (j < threadsPerBlock && size <= 1024) {
                // Use shared memory for small problems
                BitonicSort_shared<<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(d_arr, j, k, size);
            } else {
                // Use unrolled global kernel for better efficiency
                BitonicSort_global_unrolled<<<blocksPerGrid, threadsPerBlock>>>(d_arr, j, k, size);
            }
            total_launches++;
            j /= 2;
        }
    ...
    LOG_INFO("Bitonic sort completed with %d total launches (%d fused)", total_launches, fused_launches);
    LOG_INFO("Kernel launch reduction: %.1f%%", (378.0 - total_launches) / 378.0 * 100);
    ...
    if (valid) {
        ...
        printf("- Kernel launches: %d (vs 378 baseline)\n", total_launches);
        printf("- Fused kernels: %d\n", fused_launches);
    } else {
